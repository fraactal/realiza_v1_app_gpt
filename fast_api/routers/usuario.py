...
def get_usuario(my_user: Usuario = Depends(get_current_active_user)):
    try:
...
# Se debe agregar funcion para actualizar Datos, Recuperar Clave

'''@router.get('/Usuarios/Login', tags=["Usuarios"])
def login():
    try:
        # al ser un docker compose, la url lleva el nombre del servicio realiza-service
        url = f"http://realiza-service:{3000}/Usuarios"
